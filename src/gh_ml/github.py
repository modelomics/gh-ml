...
def _retry_delay(headers: Any, attempt: int) -> float:
    ...
    if raw:
        try:
            return min(max(float(raw), 0.0), _MAX_RETRY_SLEEP)
        except (TypeError, ValueError):
            try:
                retry_at = parsedate_to_datetime(raw)
                if retry_at.tzinfo is None:
                    retry_at = retry_at.replace(tzinfo=timezone.utc)
                return min(max((retry_at - datetime.now(timezone.utc)).total_seconds(), 0.0), _MAX_RETRY_SLEEP)
    ...
    if reset:
        try:
            return min(max(float(reset) - time.time(), 0.0), _MAX_RETRY_SLEEP)
        except (TypeError, ValueError):
            pass
