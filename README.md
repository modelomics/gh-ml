...
The included [GitHub Actions workflow](.github/workflows/daily.yml) runs the incremental collection and a bounded historical backfill daily at 09:17 UTC; it can also be started manually. It currently allows up to 600 GitHub Search requests for the recent sweep and 900 for backfill per workflow run (subject to the overall job timeout and API limits). Backfill resumes from `state/backfill.json` in the Hugging Face dataset; daily state uses `state/checkpoint.json`. Successful runs append observation and coverage files and update the corresponding checkpoint in one dataset commit. A successful empty sweep still publishes its coverage and checkpoint, but has no observation file. Failed searches do not publish or advance the Hub checkpoint, so rerunning a manually dispatched workflow resumes from the last successful checkpoint.

GitHub Actions supplies `GITHUB_TOKEN`. For Hugging Face publishing, the workflow supports either an `HF_TOKEN` Actions secret with write permission on `modelomics/gh-ml`, or keyless authentication through Hugging Face Trusted Publishers. To enable keyless publishing, a dataset organization writer must add a Trusted Publisher at [the dataset settings](https://huggingface.co/datasets/modelomics/gh-ml/settings) with these GitHub Actions claims: repository `modelomics/gh-ml`, branch `main`, workflow `daily.yml`. The workflow requests `id-token: write` and uses `HF_OIDC_RESOURCE=datasets/modelomics/gh-ml` to exchange its GitHub identity for a short-lived Hugging Face token. See [Hugging Face Trusted Publishers](https://huggingface.co/docs/hub/trusted-publishers). Separately, a dataset organization writer must merge the initial seed pull request (#1) to make its 36,703 seed candidates visible on `main`; this is not required for the collector to publish new runs once authentication is configured. If `HF_TOKEN` is set, it takes precedence over OIDC. For local publishing, authenticate GitHub either by exporting `GITHUB_TOKEN` or by signing in with `gh auth login` (the CLI can read `gh auth token`). Authenticate Hugging Face either by exporting `HF_TOKEN` or by running `uv run hf auth login`; the collector reads the saved Hugging Face CLI token. The HF token needs write access to the dataset. `--no-publish` only writes local output and does not require an HF token. Never commit tokens, local `.env` files, or generated data. Inspect `coverage/<run-id>.json` for capped searches, incomplete results, errors, or request-budget pauses; a successful sweep is not proof of exhaustive GitHub coverage.
