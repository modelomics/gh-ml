...
The code repository is intended for [modelomics/gh-ml on GitHub](https://github.com/modelomics/gh-ml). The collector targets the dataset repository [`modelomics/gh-ml`](https://huggingface.co/datasets/modelomics/gh-ml) on the [Modelomics Hugging Face organization](https://huggingface.co/modelomics). The dataset has not been published yet; the link is the configured destination. The GitHub Actions workflow is defined in this repository, but it only runs when installed on the default branch with `HF_TOKEN` configured.
...
`run` accepts `--repo`, `--config-dir`, `--output-dir`, `--max-requests`, `--since-days`, and `--no-publish`; use `uv run gh-ml run --help` for the exact defaults. `backfill` accepts `--repo`, `--config-dir`, `--output-dir`, `--start`, `--end`, `--max-requests`, and `--no-publish`. It searches repository creation dates, starting at `2008-01-01` by default, and splits dense date intervals when needed to get beneath GitHub’s search result cap. Each invocation respects `--max-requests`, writes coverage per query/date partition, and saves a resumable cursor. Run the same command again to continue until coverage reports the sweep complete. The default output is under `~/.local/share/modelomics-gh-ml/runs`; daily state is `state.json`, and backfill state is `backfill-state.json`.

The included [GitHub Actions workflow](.github/workflows/daily.yml) runs the incremental collection and a bounded historical backfill daily at 09:17 UTC; it can also be started manually. The current limits are 120 requests for the recent sweep and 30 for backfill per workflow run. The backfill resumes from `state/backfill.json` in the Hugging Face dataset; daily state uses `state/checkpoint.json`. Both modes append observations and query/date-partition coverage to the dataset repository. GitHub Actions supplies `GITHUB_TOKEN`; configure `HF_TOKEN` as a repository or organization Actions secret with write access to the target dataset repository. For local publishing, export both environment variables. Never commit tokens, local `.env` files, or generated data. Each run records query coverage so rate-limit and incomplete-query outcomes can be inspected. GitHub scheduled workflows run only from the default branch, can be delayed or dropped during periods of high Actions load, and are automatically disabled in public repositories after 60 days without repository activity. Use manual dispatch to recover or continue a run when needed; see [GitHub schedule event details](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule).

Local output and generated run artifacts belong outside version control. The `.gitignore` excludes local `data/` and JSONL/Parquet outputs. See the dataset card template in [`dataset/README.md`](dataset/README.md) for intended Hub files and merge behavior.
