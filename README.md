...
The included [GitHub Actions workflow](.github/workflows/daily.yml) runs the incremental collection and a bounded historical backfill daily at 09:17 UTC; it can also be started manually. The current limits are 120 requests for the recent sweep and 30 for backfill per workflow run. The backfill resumes from `state/backfill.json` in the Hugging Face dataset; daily state uses `state/checkpoint.json`. Both modes append observations and query/date-partition coverage to the dataset repository. GitHub Actions supplies `GITHUB_TOKEN`. Configure `HF_TOKEN` as a repository or organization Actions secret, and ensure that token has write permission on `modelomics/gh-ml`. The workflow checks for the secret before collection and fails fast if it is missing. For local publishing, export both environment variables and use an HF token with dataset write permission. Never commit tokens, local `.env` files, or generated data. Each run records query coverage so rate-limit and incomplete-query outcomes can be inspected. GitHub scheduled workflows run only from the default branch, can be delayed or dropped during periods of high Actions load, and are automatically disabled in public repositories after 60 days without repository activity. Use manual dispatch to recover or continue a run when needed; see [GitHub schedule event details](https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#schedule).
