...
`historical-sample` accepts `--repo`, `--config-dir`, `--output-dir`, `--max-requests`, and `--no-publish`. It takes one first-page search for each configured query and each year from 2008 through a fixed current end date, processing query groups round-robin so the request budget is spread across the catalog. Its independent local checkpoint is `historical-sample-state.json`; once complete, rerunning is a no-op until the catalog changes. This annual sample improves breadth across repository creation years, but it is ranked first-page sampling and does not replace date-partitioned historical backfill or guarantee complete coverage.
...
The included [GitHub Actions workflow](.github/workflows/daily.yml) runs four collection passes daily at 09:17 UTC and can also be started manually: a breadth sample, an incremental recent sweep, an annual historical sample, and a bounded fair historical backfill. The breadth sample issues one `created:` first-page search per configured catalog query, with a default budget of 500 requests. It is a useful broad discovery pass, not a complete census: first-page ranking, GitHub indexing, and query design leave coverage gaps. The recent sweep allows up to 600 Search requests, the annual historical sample up to 200, and fair backfill up to 700 per workflow run. The scheduled Search request budget is therefore 2,000 total, subject to the 300-minute job timeout and API limits. Annual sampling visits one first page for each configured query/year from 2008 through a fixed current end, round-robin across query groups. Fair backfill rotates through query/date-partition work, issuing one Search request per query in each rotation to distribute its request budget. Its checkpoint is separate at `state/backfill-fair.json`; daily recent state uses `state/checkpoint.json`, breadth sample uses `state/sample.json`, and annual sample uses `state/historical-sample.json`. The legacy `backfill` command and `state/backfill.json` checkpoint remain available for historical runs, but the scheduler now runs `backfill-fair`. The fair pass makes no exhaustive-coverage claim until its date partitions have been scanned and coverage records show the completed sweep. Successful passes append observation and coverage files and update their corresponding checkpoint. A successful empty pass still publishes coverage and its checkpoint, but has no observation file. Failed searches do not publish or advance the Hub checkpoint, so rerunning a manually dispatched workflow resumes from the last successful checkpoint. Dispatch inputs allow setting each pass's bounded request budget.

Run the breadth sample locally with `uv run gh-ml sample --max-requests 500 --since-days 1`. It uses the configured catalog queries and a local resumable sample checkpoint in the chosen output directory; rerun with the same output directory to continue. Add `--no-publish` to keep results local. The separate sample, recent, annual historical-sample, and backfill checkpoints let each pass resume without advancing another pass's progress.

Run the annual historical sample locally with `uv run gh-ml historical-sample --max-requests 200`. It uses a separate `historical-sample-state.json` checkpoint in the selected output directory. A completed checkpoint makes later invocations no-ops until the catalog changes. It covers only one ranked first page for each query/year, so it can miss matching repositories and is not a completeness claim. For fair historical collection without publishing, run `uv run gh-ml backfill-fair --max-requests 700 --no-publish`; its `backfill-fair-state.json` checkpoint resumes independently from the legacy `backfill` checkpoint. The old cursor is not migrated, and exhaustive coverage should not be assumed: completeness requires scanning all date partitions and confirming the coverage records show a completed sweep.

GitHub Actions supplies `GITHUB_TOKEN`. The accumulated seed and backfill observations in open Hugging Face dataset proposal #1 will appear on `main` when a Modelomics dataset writer merges it. Future collection runs can publish independently once Hugging Face authentication is configured. The workflow supports either an `HF_TOKEN` Actions secret with write permission on `modelomics/gh-ml`, or keyless authentication through Hugging Face Trusted Publishers. To enable keyless publishing, a dataset organization writer must add a Trusted Publisher at [the dataset settings](https://huggingface.co/datasets/modelomics/gh-ml/settings) with these GitHub Actions claims: repository `modelomics/gh-ml`, branch `main`, workflow `daily.yml`. The workflow requests `id-token: write` and uses `HF_OIDC_RESOURCE=datasets/modelomics/gh-ml` to exchange its GitHub identity for a short-lived Hugging Face token. See [Hugging Face Trusted Publishers](https://huggingface.co/docs/hub/trusted-publishers). If `HF_TOKEN` is set, it takes precedence over OIDC. For local publishing, authenticate GitHub either by exporting `GITHUB_TOKEN` or by signing in with `gh auth login` (the CLI can read `gh auth token`). Authenticate Hugging Face either by exporting `HF_TOKEN` or by running `uv run hf auth login`; the collector reads the saved Hugging Face CLI token. The HF token needs write access to the dataset. `--no-publish` only writes local output and does not require an HF token. Never commit tokens, local `.env` files, or generated data. Inspect `coverage/<run-id>.json` for capped searches, incomplete results, errors, or request-budget pauses; a successful sweep is not proof of exhaustive GitHub coverage.
