...
`historical-sample` accepts `--repo`, `--config-dir`, `--output-dir`, `--max-requests`, and `--no-publish`. It takes one first-page search for each configured query and each year from 2008 through a fixed current end date, processing query groups round-robin so the request budget is spread across the catalog. Its independent local checkpoint is `historical-sample-state.json`; once complete, rerunning is a no-op until the catalog changes. This annual sample improves breadth across repository creation years, but it is ranked first-page sampling and does not replace the date-partitioned exhaustive backfill or guarantee complete coverage.

The included [GitHub Actions workflow](.github/workflows/daily.yml) runs four collection passes daily at 09:17 UTC and can also be started manually: a breadth sample, an incremental recent sweep, an annual historical sample, and a bounded historical backfill. The breadth sample issues one `created:` first-page search per configured catalog query, with a default budget of 500 requests. It is a useful broad discovery pass, not a complete census: first-page ranking, GitHub indexing, and query design leave coverage gaps. The recent sweep allows up to 600 Search requests, the annual historical sample up to 200, and exhaustive backfill up to 700 per workflow run. The scheduled Search request budget is therefore 2,000 total, subject to the 300-minute job timeout and API limits. Annual sampling visits one first page for each configured query/year from 2008 through a fixed current end, round-robin across query groups. Its progress is stored separately in `state/historical-sample.json`; daily recent state uses `state/checkpoint.json`, breadth sample uses `state/sample.json`, and exhaustive backfill uses `state/backfill.json`. The annual sample improves breadth but does not replace exhaustive backfill. Successful passes append observation and coverage files and update their corresponding checkpoint. A successful empty pass still publishes coverage and its checkpoint, but has no observation file. Failed searches do not publish or advance the Hub checkpoint, so rerunning a manually dispatched workflow resumes from the last successful checkpoint. Dispatch inputs allow setting each pass's bounded request budget.
