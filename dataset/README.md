...
The scheduled workflow runs four passes for a total default Search request budget of 2,000: breadth sample (500), recent collection (600), annual historical sample (200), and exhaustive historical backfill (700), subject to Actions timeout and GitHub API limits. The breadth sample runs one `created:` first-page search for each configured catalog query. This improves breadth, but it only sees first-page results and cannot guarantee complete coverage or identify verified novelty. The recent pass searches recently pushed candidates and records the queries that found each repository. Repository searches include `fork:true` unless a configured query already specifies a `fork:` qualifier, so forks and reproductions can be discovered; `fork` marks forked projects.

The `historical-sample` pass takes one first-page search for every configured query/year from 2008 through a fixed current end date. It processes query groups round-robin so its bounded request budget is distributed across the catalog. Progress is independent at `state/historical-sample.json` on the Hub and `historical-sample-state.json` in local output. A completed checkpoint is a no-op until the catalog changes. These annual samples improve breadth across creation years, but first-page ranking can omit matching repositories; they do not replace exhaustive backfill and do not guarantee completeness. Exhaustive backfill searches creation dates beginning at 2008-01-01 by default, partitions by date, and recursively splits dense intervals when GitHub approaches its per-search result ceiling; coverage records each query and date partition. Its checkpoint is `state/backfill.json`. Recent and breadth checkpoints remain at `state/checkpoint.json` and `state/sample.json`, respectively. Within a run, matches merge by numeric `github_id`, never by mutable `name`. Across runs, the observation history is append-only; to materialize a current view, group by `github_id` and select the greatest `observed_at`. An older backfill observation does not displace a newer snapshot in a current view.
...
Coverage is per run; it is not a list of repositories. Breadth sample coverage describes one first page per catalog query; annual historical-sample coverage describes one first page per query/year; daily recent coverage describes pushed-date search windows; exhaustive backfill coverage describes created-date partitions. A `complete_sweep` of `false` usually means the request budget left a cursor to resume. Within each run's `queries`, review `status`, `coverage_gap`, `coverage_gap_reason`, `incomplete_results`, and `search_limit_reached`. Coverage and checkpoint JSON are operational metadata, not rows in the repository table. Hub checkpoints let scheduled or manually dispatched Actions runs resume; locally, reuse the same `--output-dir` to resume local state. Run the breadth sample locally with `uv run gh-ml sample --max-requests 500 --since-days 1`; run the annual sample with `uv run gh-ml historical-sample --max-requests 200`. Add `--no-publish` to keep output local.
