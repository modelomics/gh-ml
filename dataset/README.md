...
data/current/manifest.json         # source and selection counts, hashes, and provenance
...
This dataset is maintained at [`modelomics/gh-ml`](https://huggingface.co/datasets/modelomics/gh-ml). The append-only observations are available on `main`. Hugging Face Trusted Publisher authentication is configured for repository `modelomics/gh-ml`, branch `main`, and workflow `daily.yml`. The workflow requests `id-token: write` and exchanges its GitHub identity using `HF_OIDC_RESOURCE=datasets/modelomics/gh-ml`; see [Hugging Face Trusted Publishers](https://huggingface.co/docs/hub/trusted-publishers). An `HF_TOKEN` secret, when set, takes precedence over OIDC. Actions supplies `GITHUB_TOKEN` for GitHub Search. The `current` config points to the strict Parquet view and will become available with its first atomic snapshot/card publication; until then, the remote Hub may not yet have the file/config. To recover a scheduled Search run, use **Run workflow** in Actions: each successful pass commits its cursor with observations and coverage, while a failed search leaves the prior checkpoint available for retry.
...
The dataset-card YAML below declares `current` as the default configuration and `observations` as the named, opt-in raw history. The source card will be uploaded atomically with the first strict Parquet artifact; the remote card is not yet confirmed updated. These configs follow the Hub's [dataset repository structure](https://huggingface.co/docs/datasets/repository_structure); `datasets` is needed only by consumers, not by the collector.
